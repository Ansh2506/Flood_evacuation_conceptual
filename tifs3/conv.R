...
res <- 24
#import TIFF rasters
rvr_raster <- raster("rvr.tif")
bld_raster <- raster("bld.tif")
rds_raster <- raster("rds.tif")
...
for(i in 2:dim(dat)[1]-1){
  for(j in 2:dim(dat)[2]-1){
    ...
    else if (dat[i, j] == 3) txt[i,j] <- "P"
    
    #river boundary
    if( (dat[i,j-1] == 0) && (dat[i,j] == 1)){
      txt[i,j] <- "W"
    }
    else if( (dat[i, j+1] == 0) && (dat[i,j] == 1)){
      txt[i,j] <- "W"
    }
    else if((dat[i,j] == 0) && (dat[i+1,j] == 1)) txt[i,j] <- "W"
